...
from dict import dict1_build,dict1_search,dict2_build,dict2_search
...
def hashSearch():
    (delay_build,dictation) = dict2_build()
...
# choice = 1 map, choice = 2 hash
def searchHandler(dictionary):
    word = str(entry.get())
    if choice_dictionary.get() == 1:
        (delay2,value) = dict1_search(dictionary,word)
    elif choice_dictionary.get() == 2:
        (delay2,value) = dict2_search(dictionary,word)
